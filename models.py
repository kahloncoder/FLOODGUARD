...
class RainfallData(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    district = Column(String(50), nullable=False)
    location = Column(Geometry('POINT', srid=4326), nullable=False)  # District center point
    rainfall_mm = Column(Float, nullable=False)  # Rainfall in millimeters
    duration_hours = Column(Integer, nullable=False)  # Duration over which rainfall was measured
    timestamp = Column(DateTime, default=datetime.utcnow)
